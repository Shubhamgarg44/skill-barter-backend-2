...
export const acceptSkillRequest = async (req, res) => {
  try {
    ...
    request.completionCode = completionCode;
...
export const verifyCompletionCode = async (req, res) => {
  try {
    const { requestId } = req.params;
    const { code } = req.body;
...
export const getRecommendations = async (req, res) => {
  try {
    // Get user's completed/requested skills
    const requests = await SkillRequest.find({
      requester: req.user.id,
    }).populate("skill");

    if (requests.length === 0) {
      // New user → show latest skills
      const latestSkills = await Skill.find()
        .populate("offeredBy", "name")
        .sort({ createdAt: -1 })
        .limit(5);

      return res.json(latestSkills);
    }
